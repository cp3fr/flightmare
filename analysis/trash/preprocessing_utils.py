...
def track_from_json(fpn):
    ...
    '''
    world = fpn.split('/')[-2]
    track = fpn.split('/')[-1].split('.json')[0]
    df = pd.DataFrame()
    a = {}
    with open(fpn) as f:
...
def load_tracks(PATH):
    ...
    files = {'hairpin':['slalom.json', [3,9], [0,0]],
             ...
             'flat': ['flat.json', [7, 8, 9, 6, 1, 4, 3, 2, 5, 0], [1, 0, 0, 0, 0, 0, 0, 1, 1, 1]],
             'wave': ['wave.json', [7, 8, 9, 6, 1, 4, 3, 2, 5, 0], [1, 0, 0, 0, 0, 0, 0, 1, 1, 1]]}
             ##old way: starting at the podium
    ...
    for key, value in files.items():
        ...
        track['Gate [id]']=np.arange(1, track.shape[0]+1, 1) #start counting from 1
...
def process_run(PATH, run, tracks, drone, pupil, screen, clocktime_offset=0., export_files=['all']):
    ...
    if curr_pupil.size and curr_screen.size:
        ...
        if (outpath.find('splits') != -1) or (outpath.find('flat') != -1) or (outpath.find('wave') != -1):
            ...
            obj.run()
        #perform ray tracing, which saves the gaze intersections in 'gaze_traces.csv'
        tracer = RayTracer(outpath)

def import_trpy_data(D, rates_path='../data/raw/s004/airr-sim/data/rates.csv', thrust_path='../data/raw/s004/airr-sim/data/thrust_single_motor.csv',
                     already_wrongly_normalized=False):
    '''
    D: pandas dataframe of raw drone state logs, including the raw data columns ['Throttle', 'Roll', 'Pitch', 'Yaw]
        where Throttle in range [-992, 992] and Roll, Pitch, Yaw in range [-817, 817]
    already_wrongly_normalized: a flag to fix if data was already normalized (correct for Thrust) or (wrong for RPY) before
    rates_path: path to the 'rates.csv' file of the alphapilot simulator
    thrust_path: path to the 'thrust_single_motor.csv' file of the alphapilot simulator
    '''
    if os.path.isfile(rates_path) and os.path.isfile(thrust_path):
        ##================
        ## Throttle import
        #load the thrust lookup table
        T = pd.read_csv(thrust_path, skiprows=1, header=None, sep=';')
        T.columns = ['rc_signal', 'norm_signal', 'thrust_N', 'thrust_G']
        #name of the variable
        name = 'Throttle'
        #if the data was already wrongly normalized, convert it back to raw values
        if already_wrongly_normalized:
            D[name] = ((D[name].values * 2.) -1.) * 992.
        # add raw data with range again
        varname = name.lower() + '_raw [-992,992]'
        D[varname] = D[name].values
        # add normalized data
        varname = name.lower() + '_norm [0,1]'
        D[varname] = ((D[name].values / 992.) / 2.) + 0.5
        # take the normalized data
        N = D[varname].values
        # now convert to angular rate in rad/s
        A = np.empty(N.shape)
        A[:] = np.nan
        #input command steps
        delta = np.nanmedian(np.diff(T.norm_signal.values))
        #loop over command steps
        for i in range(T.shape[0]):
            # minimum value for current normalized input
            min_val = T.norm_signal.iloc[i]
            # maximum value for current normalized input
            if i == (T.shape[0] - 1):
                max_val = T.norm_signal.iloc[i] + delta
            else:
                max_val = T.norm_signal.iloc[i + 1]
            # pointer to all values within the min max input range
            ind = (np.abs(N) >= min_val) & (np.abs(N) < max_val)
            # set current thrust value
            A[ind] = T['thrust_N'].iloc[i]
        # add data as commanded thrust [N]
        varname = name.lower() + '_thrust [N]'
        D[varname] = A
        ##=====================
        ## Angular Rates import
        #load the rates lookup table
        R = pd.read_csv(rates_path, skiprows=1, header=None, sep=';')
        R.columns = ['norm_signal', 'rc_signal', 'angular_rate_deg/s', 'angular_rate_ai_deg/sec']
        #add rates in radians per second
        R['angular_rate_rad/s'] = np.deg2rad(R['angular_rate_deg/s'].values)
        #loop over angular rates
        for name in ['Roll', 'Pitch', 'Yaw']:
            # if the data was already wrongly normalized, convert it back to raw values
            if already_wrongly_normalized:
                D[name] = D[name].values * 992.
            #add raw data with range again
            varname = name.lower() + '_raw [-817,817]'
            D[varname] = D[name].values
            #add normalized data
            varname = name.lower() + '_norm [-1,1]'
            D[varname] = D[name].values / 817.
            #take the normalized data
            N = D[varname].values
            # now convert to angular rate in rad/s
            A = np.empty(N.shape)
            A[:] = np.nan
            #this is the step size between normalized inputs
            delta = np.nanmedian(np.diff(R.norm_signal.values))
            #loop over input  steps
            for i in range(R.shape[0]):
                # minimum value for current normalized input
                min_val = R.norm_signal.iloc[i]
                # maximum value for current normalized input
                if i == (R.shape[0] - 1):
                    max_val = R.norm_signal.iloc[i] + delta
                else:
                    max_val = R.norm_signal.iloc[i + 1]
                # pointer to all values within the min max input range
                ind = (np.abs(N) >= min_val) & (np.abs(N) < max_val)
                # set current absolute angular rate to output vector
                A[ind] = R['angular_rate_rad/s'].iloc[i]
                # find negative input values
                ind = ind & (N < 0.)
                # set output values to min max
                A[ind] = -A[ind]
            # add data as angular rates [rad/s]
            varname = name.lower() + '_rate [rad/s]'
            D[varname] = A
        # return the converted data
        return D

def angular_rate_to_setpoint(M):
    #assuming raw data [-817, +817] range was assumed in [-992;+992] range and normalized to [-1;1] range
    #but actually it should have been normalized to [-817;+817] range
    #load the rate conversion information
    inpath = '../data/raw/s004/airr-sim/data/rates.csv'
    C = pd.read_csv(inpath, skiprows=1, header=None, sep=';')
    C.columns = ['norm_signal', 'rc_signal', 'angular_rate_deg/s', 'angular_rate_ai_deg/sec']
    C['angular_rate_rad/s'] = np.deg2rad(C['angular_rate_deg/s'].values)
    #convert to correct normalization
    N = (M * 992.) / 817.
    #now convert to angular rate in rad/s
    A = np.empty(N.shape)
    A[:] = np.nan
    delta = np.nanmedian(np.diff(C.norm_signal.values))
    for i in range(C.shape[0]):
        #minimum value for current normalized input
        min_val = C.norm_signal.iloc[i]
        #maximum value for current normalized input
        if i == (C.shape[0]-1):
            max_val = C.norm_signal.iloc[i] + delta
        else:
            max_val = C.norm_signal.iloc[i + 1]
        #pointer to all values within the min max input range
        ind = (np.abs(N) >= min_val) & (np.abs(N) < max_val)
        #set current absolute angular rate to output vector
        A[ind] = C['angular_rate_rad/s'].iloc[i]
        #find negative input values
        ind = ind & (N < 0.)
        #set ouput values to min max
        A[ind] = -A[ind]
    #return the converted data
    return A

def throttle_to_setpoint(N):
    #assuming raw data [-992 +992] was normalized to [0;1] range
    #load the rate conversion information
    inpath = '../data/raw/s004/airr-sim/data/thrust_single_motor.csv'
    C = pd.read_csv(inpath, skiprows=1, header=None, sep=';')
    C.columns = ['rc_signal', 'norm_signal', 'thrust_N', 'thrust_G']
    #now convert to angular rate in rad/s
    A = np.empty(N.shape)
    A[:] = np.nan
    delta = np.nanmedian(np.diff(C.norm_signal.values))
